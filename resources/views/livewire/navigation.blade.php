  ...
  <div class="mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between h-16">
      <div class="flex flex-wrap">
        ...
        <!-- Vehicle Dropdown -->
        @canany(array_map(fn($item) => $item->permission, $this->vehicleNavs))
          <div class="hidden sm:-my-px sm:mr-2 sm:flex">
            <x-nav-link-dropdown label="Vehicle" :active="request()->routeIs(['vehicle','vehicle.show','vehicle.create','vehicle.edit','vmodel','vtype', 'brand'])"  width="auto">
              <x-slot name="content">
                @foreach ($this->vehicleNavs as $data)
                  @can($data->permission)
                    <x-dropdown-link :href="route($data->route)" :active="request()->routeIs($data->route)" wire:navigate>
                      {{ __($data->label) }}
                    </x-dropdown-link>
                  @endcan
                @endforeach
        ...
        <!-- Financial Dropdown -->
        @canany(array_map(fn($item) => $item->permission, $this->financialNavs))
          <div class="hidden sm:-my-px sm:mr-2 sm:flex">
            <x-nav-link-dropdown label="Financial" :active="request()->routeIs(['sale.create', 'sale.show', 'sales', 'sale.installments', 'installments', 'vehicle-expense'])"  width="auto">
              <x-slot name="content">
                @foreach ($this->financialNavs as $data)
                  @can($data->permission)
                    <x-dropdown-link :href="route($data->route)" :active="request()->routeIs($data->route)" wire:navigate>
                      {{ __($data->label) }}
                    </x-dropdown-link>
